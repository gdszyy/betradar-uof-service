...
// NotifyLiveDataConnected 发送 Live Data 连接成功通知
func (n *LarkNotifier) NotifyLiveDataConnected(server string) error {
	if !n.enabled {
		return nil
	}
	...
	content := [][]LarkElement{
		{
			{Tag: "text", Text: "🟢 Live Data 已连接\n"},
		},
		{
			{Tag: "text", Text: fmt.Sprintf("服务器: %s\n", server)},
		},
		{
			{Tag: "text", Text: fmt.Sprintf("时间: %s", time.Now().Format("2006-01-02 15:04:05"))},
		},
	...
	return n.SendRichText("Live Data Status", content)
}

// NotifyLiveDataDisconnected 发送 Live Data 断线通知
func (n *LarkNotifier) NotifyLiveDataDisconnected(reason string) error {
	if !n.enabled {
		return nil
	}
	...
	return n.SendRichText("Live Data Alert", content)
}
